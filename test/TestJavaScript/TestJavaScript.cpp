...
void TestJavaScript::testInterpolation_data()
{
    ...
    QTest::newRow("empty") << "|" << indentPart("", 0) + newLine;
    QTest::newRow("simple") << "|simple " << indentPart("", 0) + output("simple ", 1) + newLine;
    QTest::newRow("indentation space") << "\t| simple" << indentPart(" ", 0) + output("simple", 1) + newLine;
    QTest::newRow("indentation tab") << " |\tsimple" << indentPart("\t", 0) + output("simple", 1) + newLine;

    QTest::newRow("no expr") << "|#(test" << indentPart("", 0) + output("#(test", 1) + newLine;
    QTest::newRow("escaped expr") << "|##{test" << indentPart("", 0) + output("#", 1) + output("{test", 2) + newLine;
    QTest::newRow("single expr 1") << "|#{test}" << indentPart("", 0) + expression("test", 1) + newLine;
    QTest::newRow("single expr 2") << "|#{test}}" << indentPart("", 0) + expression("test", 1) + output("}", 2) + newLine;
    QTest::newRow("inline expr 1") << "|simple #{test.prop} expr" << indentPart("", 0) + output("simple ", 1) + expression("test.prop", 2) + output(" expr", 3) + newLine;
    QTest::newRow("logic expr") << "|#{ (i==0 ? ':' : \",\") }" << indentPart("", 0) + expression(" (i==0 ? ':' : \",\") ", 1) + newLine;
    QTest::newRow("escaped expr 1") << "|escape ##{test} expr" << indentPart("", 0) + output("escape #", 1) + output("{test} expr", 2) + newLine;
    QTest::newRow("escaped expr 2") << "|###{test}" << indentPart("", 0) + output("#", 1) + expression("test", 2) + newLine;
    QTest::newRow("string expr 1") << "|#{\"}\"}" << indentPart("", 0) + expression("\"}\"", 1) + newLine;
    QTest::newRow("string expr 2") << "|#{'\\'\\\"}'}" << indentPart("", 0) + expression("'\\'\\\"}'", 1) + newLine;
